...
router.post('/', requireRecipeEditor, ah(async (req, res) => {
  const { nom, description = null, portion_base = 10, allergenes = null, allergenes_libre = null, ingredients = [], materiels = [] } = req.body || {};
  if (!nom) return res.status(400).json({ error: 'nom requis' });
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { rows } = await client.query(
      'INSERT INTO recettes (nom, description, portion_base, allergenes, allergenes_libre) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [nom, description, portion_base, allergenes, allergenes_libre]);
...
router.put('/:id', requireRecipeEditor, ah(async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const { rows } = await client.query(
      'UPDATE recettes SET nom = $1, description = $2, portion_base = $3, allergenes = $4, allergenes_libre = $5 WHERE id = $6 RETURNING *',
      [nom, description, portion_base, allergenes, allergenes_libre, req.params.id]);
